...
function generatableProfesores(data) {
    ...
    let tabla = generarTablaCargueInventario(
        ...
        [
            ...
            {
                valor: "Informaci&oacute;n General", size: "30"
            },
            { valor: "Informaci&oacute;n M&eacute;dica", size: "30" },
            { valor: "Informaci&oacute;n Socioecon&oacute;mica", size: "25" },
...
function generarTablaCargueInventario(
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        contenedor += `<a href="#" class="form-control form-control-sm bg-primary text-white" data-toggle="modal" onclick ="ModalCertificadosEstudianteBoletin(${data[i][propiedadId]})" data-target="#BoletinesModal">Boletines</a>
                            <a href="#" class="form-control form-control-sm bg-primary text-white" data-toggle="modal" data-target="#Registrobservacion" onclick ="ModalCertificadosEstudiante(${data[i][propiedadId]})">Certificados
                            </a>`;
        contenedor += "</td>";
        contenedor += "</tr>";
    }
...
function filtrosEstudiantes() {
    ...
    fetchGet(`Funcionario/ListarEstudiantesSedeGrupo/?sede=${sedes}&grupo=${grupos}&grado=${grados}&identificacion=${idetificacion}`, function (data) {
...
function ModalCertificadosEstudianteBoletin(documento) {

    document.getElementById("identificacionEstudanteBoletin").value = documento;
}

function generarCertificadoEstudioBoletin() {
    let documento = document.getElementById("identificacionEstudanteBoletin").value;

    fetchGetText('Funcionario/retornarInfoFamiliaresEstudianteXCertificados/?identificacion=' + documento, function (res) {
        if (res === "1") {
            fetchGet('estudiante/InfoBoletinEstudianteIdentificacion/?idEstudiante=' + documento, function (res1) {

                if (res1.grado === "0") {
                    document.location.href = setUrl('pdf/Index/?RutaPdf=formatoPreescolarBoletin3');
...
function ModalCertificadosEstudiante(documento) {

    document.getElementById("identificacionEstudante").value = documento;  
}

function generarCertificadoEstudio()
{
    let documento = document.getElementById("identificacionEstudante").value;

    fetchGetText('Funcionario/retornarInfoFamiliaresEstudianteXCertificados/?identificacion=' + documento, function (res) {
        if (res === "1") {
            fetchGetText('estudiante/InfoBoletinEstudianteIdentificacion/?idEstudiante=' + documento, function (res1) {

                document.location.href = setUrl('pdf/Index/?RutaPdf=formatoCertificadoEstudio');  
...
function generarCertificadoEstudiante(formato) {
    let documento = document.getElementById("identificacionEstudante").value;
